...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
  while cap.isOpened():
    ...
    head = canvas.create_oval(head_x1, head_y1, head_x2, head_y2, fill = "orange")
    canvas.create_arc(head_x1, head_y1, head_x2, head_y2 - 3*eye_size, start = 0, extent = 180, fill = "orange")
    ...
    # Sholder
    shoulder_mid_x = head_center_x
    shoulder_mid_y = head_center_y + head_r + neck_size
    shoulder_x1 = shoulder_mid_x - (shoulder_length / 2)
    shoulder_y1 = shoulder_mid_y
    shoulder_x2 = shoulder_mid_x + (shoulder_length / 2)
    shoulder_y2 = shoulder_mid_y

    # Body
    body_y2 = shoulder_y2 + body_height
    body = canvas.create_rectangle(shoulder_x1, shoulder_y1, shoulder_x2, body_y2, fill = "red")

    # Hands
    L_hand_x = shoulder_x1
    L_hand_y = shoulder_y1
    R_hand_x = shoulder_x2
    R_hand_y = shoulder_y2
    ...
    # Hand Fingers
    Lh_finger_x1 = L_hand_end_x - finger_size
    Lh_finger_y1 = L_hand_end_y - finger_size
    Lh_finger_x2 = L_hand_end_x + finger_size
    Lh_finger_y2 = L_hand_end_y + finger_size

    Rh_finger_x1 = R_hand_end_x - finger_size
    Rh_finger_y1 = R_hand_end_y - finger_size
    Rh_finger_x2 = R_hand_end_x + finger_size
    Rh_finger_y2 = R_hand_end_y + finger_size

    canvas.create_oval(Lh_finger_x1, Lh_finger_y1, Lh_finger_x2, Lh_finger_y2, fill = "green")
    canvas.create_oval(Rh_finger_x1, Rh_finger_y1, Rh_finger_x2, Rh_finger_y2, fill="green")

    # Legs
    L_leg_x = shoulder_x1
    L_leg_y = shoulder_y1 + body_height

    R_leg_x = shoulder_x2
    R_leg_y = shoulder_y2 + body_height
    ...
    # Leg Fingers
    Ll_finger_x1 = L_leg_end_x - finger_size
    Ll_finger_y1 = L_leg_end_y - finger_size
    Ll_finger_x2 = L_leg_end_x + finger_size
    Ll_finger_y2 = L_leg_end_y + finger_size

    Rl_finger_x1 = R_leg_end_x - finger_size
    Rl_finger_y1 = R_leg_end_y - finger_size
    Rl_finger_x2 = R_leg_end_x + finger_size
    Rl_finger_y2 = R_leg_end_y + finger_size

    canvas.create_oval(Ll_finger_x1, Ll_finger_y1, Ll_finger_x2, Ll_finger_y2, fill="black")
    canvas.create_oval(Rl_finger_x1, Rl_finger_y1, Rl_finger_x2, Rl_finger_y2, fill="black")

    tk.update()
    time.sleep(0.01)
    canvas.delete('all')

    cv2.imshow('Subin', image)
